...
filling=6 #1 is filling one slice between each skipping 
t=100
...
model = LitModel(conf)
# breakpoint()
state = torch.load(f'/research/cbim/medical/xh172/diffu/med256_autoenc/last.ckpt', map_location=device)
...
base_path="/filer/tmp1/xh172/ukbb/test_extracted/"
# img_path="/filer/tmp1/xh172/"
save_to_path="/filer/tmp1/xh172/generated_ukbb/no_inter_test/T{0}/".format(t)
...
for i in tqdm(files):
    ...
    xTs=model.encode_stochastic(input_tensor,cond_tensor,T=t)
    pred = model.render(xTs,cond=cond_tensor ,T=t)
