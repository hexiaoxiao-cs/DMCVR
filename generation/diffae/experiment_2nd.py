...
class LitModel(pl.LightningModule):
    def __init__(self, conf: TrainConfig):
        ...
        # Initial Seg style, will change to the first 
        self.register_buffer(
            "seg_style",
            torch.randn(conf.sample_size, 512, 16,16)
        )
    ...
    def render(self, noise, cond=None, seg_style=None, T=None):
        if T is None:
            sampler = self.eval_sampler
        else:
            sampler = self.conf._make_diffusion_conf(T).make_sampler()
    ...
    def training_step(self, batch, batch_idx):
        ...
        with amp.autocast(False):
            ...
            if self.conf.train_mode.require_dataset_infer():
                ...
                imgs, idxs,cond = batch['img'], batch['index'],batch["latent"]#Changed ARIST: Get the SEG LATENT
    ...
    def on_train_batch_end(self, outputs, batch, batch_idx: int,
        ...
        if self.is_last_accum(batch_idx):
            ...
            if self.conf.train_mode.require_dataset_infer():
                ...
                imgs = batch['img']
                seg_style=batch["latent"]

            self.log_sample(x_start=imgs,seg_style=seg_style)
    ...
    def log_sample(self, x_start,seg_style):
        """
            ...
            with torch.no_grad():
                ...
                for x_T in loader:
                    if use_xstart:
                        ...
                        _seg_style=seg_style[:len(seg_style)]
                    else:
                        _xstart = None
                        _seg_style=None
