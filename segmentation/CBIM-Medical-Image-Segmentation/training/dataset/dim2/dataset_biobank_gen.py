...
class CMRDataset(Dataset):
    def __init__(self, args, mode='train', k_fold=5, k=0, seed=0):
        ...
        img_name_list=glob.glob(os.path.join(args.data_root,"lbl_clean","*.nii.gz"))
        ...
        for name in tqdm.tqdm(img_name_list):
            ...
            bn_img=bn.replace("_label_sa","_sa")

            img_name=os.path.join(args.data_root,"img",bn_img)
            lbl_name=name
            if not os.path.exists(img_name):
                continue
            
            itk_img=sitk.ReadImage(img_name)
            itk_lbl=sitk.ReadImage(lbl_name)

            img=sitk.GetArrayFromImage(itk_img)
            lbl=sitk.GetArrayFromImage(itk_lbl)
            ###Here because our generated images is in range 0.5,1, we need to reregularize the images (may be temporary, need to update)
            img=img-0.5
            img=img*2
            img=np.clip(img,0,1)
            ...
            tensor_lab = torch.from_numpy(lbl).long()
            img_list.extend(list(tensor_img))
            lab_list.extend(list(tensor_lab))
            # for idx in [0, 1]:
    ...
    def __getitem__(self, idx):
        ...
        if self.mode == 'train':
            ...
            tensor_img, tensor_lab = tensor_img.unsqueeze(0), tensor_lab.unsqueeze(0)
        ...
        if self.mode == 'train':
            ...
            return tensor_img, tensor_lab,  np.array([10.0,1.826923131942749, 1.826923131942749])
